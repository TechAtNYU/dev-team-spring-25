    ...
    <div className={styles.card}>
      <div className={styles.cardHeader}>
        <div className={styles.headerContent}>
          ...
          <span className={isConnected ? styles.badgeConnected : styles.badgeDisconnected}>
        ...
        <div className={styles.messageContainer}>
          {isConnecting ? (
            <div className={styles.loadingContainer}>
              ...
              <span className={styles.loadingText}>Connecting to chat server...</span>
            </div>
          ) : messages.length === 0 ? (
                ...
                >
                  <div
                    className={`${styles.messageGroup} ${
                      msg.isCurrentUser ? styles.messageGroupRight : styles.messageGroupLeft
                    }`}
                  >
                    <div className={`${styles.avatar} ${msg.isCurrentUser ? styles.avatarRight : styles.avatarLeft}`}>
                      {msg.sender.substring(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <div className={styles.messageInfo}>
                        <span className={styles.senderName}>{msg.isCurrentUser ? "You" : msg.sender}</span>
                        <span className={styles.timestamp}>{formatTime(msg.timestamp)}</span>
                      </div>
                      <div
                        className={`${styles.messageBubble} ${
                          msg.isCurrentUser ? styles.messageBubbleSent : styles.messageBubbleReceived
                        }`}
                      >
                        {msg.text}
                      </div>
      ...
      <div className={styles.cardFooter}>
        ...
        >
          ...
          />
          <button type="submit" disabled={!isConnected || !message.trim()} className={styles.sendButton}>
